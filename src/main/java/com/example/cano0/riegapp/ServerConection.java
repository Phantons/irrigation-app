...
public class ServerConection {
    public static String[] consultarAlServidor(String mensajeEnviar, int numeroDeRespuestas){
        for(int j1 = 0; j1 < 3; j1 = j1 + 1){
            String[] respuestaDelServidor = ServerConection.consultarAlServidorProfundo(mensajeEnviar, numeroDeRespuestas);
    ...
    private static String[] consultarAlServidorProfundo(String mensajeEnviar, int numeroDeRespuestas){
        ...
        try{
            socket = new Socket();
            socket.connect(new InetSocketAddress(InetAddress.getByName("telecobets.ddns.net"), 4321), 5000);
            ...
            ServerConection.enviarMensaje(mensajeEnviar, out, aes);
            aes.init(Cipher.DECRYPT_MODE, key);
            salida = new String[numeroDeRespuestas];
            for(int j1 = 0; j1 < numeroDeRespuestas; j1 = j1 + 1){
                salida[j1] = ServerConection.recibirMensaje(in, aes);
            }
            return salida;
        }catch(Exception e){
